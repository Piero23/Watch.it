...
export class DatabaseService {
  ...
  async aggiornaStatus(tipo: string, id: number, status: number){
    //tipo: movie/tv
    await this.http.get('http://localhost:8080/contenuto/updateStatus/'+tipo+':'+id+'-'+status).toPromise();
  }

  async aggiornaStagione(id: number, stagione: number, episodio: number){
    //tipo: movie/tv
    await this.http.get('http://localhost:8080/contenuto/updateSeries/'+id+'-s:'+stagione+'-e:'+episodio).toPromise();
  }

  //Commento

  async setAnswer(questionId: number, answerId: number){
    await this.http.get('http://localhost:8080/commento/setAnswer/q:'+questionId+'-a:'+answerId).toPromise();
  }
